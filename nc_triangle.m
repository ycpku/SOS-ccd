function [lstar, ustar, diagnostics, eig_vals] = nc_triangle(P1, P2, V1, V2, n)
    ...
    X1 = trimapX(P1, u1, v1, n) + t * trimapX(V1, u1, v1, n);
    X2 = trimapX(P2, u2, v2, n) + t * trimapX(V2, u2, v2, n);
    hi = X1 - X2;
    C1 = sos(hi*hi' - lambda - [s1, s2, s3, s4, s5, s6, s7] * gi);
    C2 = [sos(s1); sos(s2); sos(s3); sos(s4); sos(s5); sos(s6); sos(s7)];
    [C, obj] = sosmodel([C1; C2; 0<=lambda], -lambda, ...
        [], [s1c; s2c; s3c; s4c; s5c; s6c; s7c; lambda]);
