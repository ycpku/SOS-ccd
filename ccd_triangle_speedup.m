function [lstar, ustar, diagnostics, eig_vals] = ccd_triangle_speedup(P1, P2, V1, V2, n)
    ...
    [s7, s7c] = polynomial([u1, v1, u2, v2, t], d1);
    [p1, p1c] = polynomial([u1, v1, u2, v2, t], d2);
    [p2, p2c] = polynomial([u1, v1, u2, v2, t], d2);
    [p3, p3c] = polynomial([u1, v1, u2, v2, t], d2);
    gi = [u1; v1; 1-u1-v1; u2; v2; 1-u2-v2; t*(1-t)]; % higher degree description
    X1 = trimapX(P1, gi(1:3),n) + t * trimapX(V1, gi(1:3),n);
    X2 = trimapX(P2, gi(4:6),n) + t * trimapX(V2, gi(4:6),n);
    hi = X1 - X2;
    C1 = sos(t - lambda - [p1, p2, p3] * hi - [s1, s2, s3, s4, s5, s6, s7] * gi);
    C2 = [sos(s1); sos(s2); sos(s3); sos(s4); sos(s5); sos(s6);sos(s7)];
    C3 = [0<=lambda; lambda<=1]; % constraining lambda
    [C, obj] = sosmodel([C1; C2; C3], -lambda, ...
        [], [s1c; s2c; s3c; s4c; s5c; s6c; s7c; p1c; p2c; p3c; lambda]);
    diagnostics = optimize(C, obj, sdpsettings('solver','mosek','verbose',0));
    lstar = value(lambda);
    mu = dual(C(4));
    eig_vals = eig(mu);
    ustar = mu(2:6)/mu(1);
...
function X = trimapX(P, g, n)
    if(n==1)
        X = P(:,1)*g(3) + P(:,2)*g(1) + P(:,3)*g(2);
    elseif(n==2)
        X = P(:,1)*g(3)^2 + 2*P(:,2)*g(1)*g(3) + P(:,3)*g(1)^2 + ...
            2*P(:,4)*g(2)*g(3) + 2*P(:,5)*g(1)*g(2) + P(:,6)*g(2)^2;
    else
        X = P(:,1)*g(3)^3 + 3*P(:,2)*g(1)*g(3)^2 + 3*P(:,3)*g(1)^2*g(3) + ...
            P(:,4)*g(1)^3 + 3*P(:,5)*g(2)*g(3)^2 + ...
            6*P(:,6)*g(1)*g(2)*g(3) + 3*P(:,7)*g(1)^2*g(2) + 3*P(:,8)*g(2)^2*g(3) + ...
            3*P(:,9)*g(1)*g(2)^2 + P(:,10)*g(2)^3;
    end
end